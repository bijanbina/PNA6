...
while true; do
	echo "-------------------------------------"
	echo "0 - Install board files in Vivado"
	echo "1 - Add Project to Analog Device HDL Repository"
	echo "2 - synthesize projects"
	echo "3 - Modify Meta-ADI project"
	echo "4 - Create Petalinux project"
	echo "-------------------------------------"
	read response_main
	...
	if [[ "$response_main" == *"0"* ]]; then
		cd $CURRENT_DIR
		echo "-------------------------------------"
		echo "0 - Install PNA6 board files in Vivado"
		echo "1 - Install E310 board files in Vivado"
		echo "-------------------------------------"
		read response_board

		if [[ "$response_board" == *"0"* ]]; then
		...
		if [[ "$response_board" == *"1"* ]]; then
			...
			#DIR="$XILINX_INSTALL_DIR/Vivado/$XILINX_VERSION/data/boards/board_parts/e310/1.0"
			#if [ ! -d "$DIR" ]; then
			#	mkdir "$XILINX_INSTALL_DIR/Vivado/$XILINX_VERSION/data/boards/board_parts/e310/1.0"
			#fi
	...
	if [[ "$response_main" == *"1"* ]]; then
		cd $CURRENT_DIR
		echo "-------------------------------------"
		echo "0 - Add PNA6 Project to Analog Device HDL Repository"
		echo "1 - Add FFT Project to Analog Device HDL Repository"
		echo "2 - Add ETTUS Project to Analog Device HDL Repository"
		echo "-------------------------------------"
		read response_project

		if [[ "$response_project" == *"0"* ]]; then
		...
		if [[ "$response_project" == *"2"* ]]; then
			DIR="$ADI_HDL_PROJECT/projects/ettus"
			if [ ! -d "$DIR" ]; then
				mkdir "$ADI_HDL_PROJECT/projects/ettus"
			fi
			DIR="$ADI_HDL_PROJECT/projects/ettus/e310"
			if [ ! -d "$DIR" ]; then
				mkdir "$ADI_HDL_PROJECT/projects/ettus/e310"
			fi
			DIR="$ADI_HDL_PROJECT/projects/ettus/common"
			if [ ! -d "$DIR" ]; then
				mkdir "$ADI_HDL_PROJECT/projects/ettus/common"
			fi
			DIR="$ADI_HDL_PROJECT/projects/common/e310"
			if [ ! -d "$DIR" ]; then
				mkdir "$ADI_HDL_PROJECT/projects/common/e310"
			fi
			DIR="$ADI_HDL_PROJECT/library/edge_detect"
			if [ ! -d "$DIR" ]; then
				mkdir "$ADI_HDL_PROJECT/library/edge_detect"
			...
			cp HDL/ettus/e310/Makefile 				"$ADI_HDL_PROJECT/projects/ettus/e310"
			cp HDL/ettus/e310/system_bd.tcl 		"$ADI_HDL_PROJECT/projects/ettus/e310"
			cp HDL/ettus/e310/system_constr.xdc 	"$ADI_HDL_PROJECT/projects/ettus/e310"
			cp HDL/ettus/e310/system_project.tcl 	"$ADI_HDL_PROJECT/projects/ettus/e310"
			cp HDL/ettus/e310/system_top.v 			"$ADI_HDL_PROJECT/projects/ettus/e310"

			cp HDL/ettus/common/ettus_qsys.tcl 		"$ADI_HDL_PROJECT/projects/ettus/common"
			cp HDL/ettus/common/ettus_bd.tcl 		"$ADI_HDL_PROJECT/projects/ettus/common"

			cp HDL/common/fft/edge_detect.v 		"$ADI_HDL_PROJECT/library/edge_detect"

			cp HDL/common/e310/e310_system_bd.tcl 		"$ADI_HDL_PROJECT/projects/common/e310"
			cp HDL/common/e310/e310_system_constr.xdc 	"$ADI_HDL_PROJECT/projects/common/e310"
			...
			if [ -z "$CHECK_PRE" ]; then
				WRITE_LN=$(grep -i -n 'regexp "_zc702$" $project_name' "$ADI_HDL_PROJECT/projects/scripts/adi_project_xilinx.tcl" | awk -F : '{printf $1}')
				WRITE_LN=$(($WRITE_LN+5))
				sed -i "$WRITE_LN"'i\ \ if [regexp "_e310$" $project_name] {' "$ADI_HDL_PROJECT/projects/scripts/adi_project_xilinx.tcl"
				WRITE_LN=$(($WRITE_LN+1))
				sed -i "$WRITE_LN"'i\ \ \ \ set p_device "xc7z020clg484-3"' "$ADI_HDL_PROJECT/projects/scripts/adi_project_xilinx.tcl"
				WRITE_LN=$(($WRITE_LN+1))
	...
	if [[ "$response_main" == *"2"* ]]; then
		echo "-------------------------------------"
		echo "0 - synthesize PNA6 project"
		echo "1 - synthesize FFT project"
		echo "2 - synthesize E310 project"
		echo "-------------------------------------"
		read response_synthesize

		if [[ "$response_synthesize" == *"0"* ]]; then
			cd "$ADI_HDL_PROJECT/projects/pna6/ddrless"
			export PATH="$PATH:$XILINX_INSTALL_DIR/Vivado/$XILINX_VERSION/bin"
			make
		fi

		if [[ "$response_synthesize" == *"1"* ]]; then
			cd "$ADI_HDL_PROJECT/projects/fft/zc702"
			export PATH="$PATH:$XILINX_INSTALL_DIR/Vivado/$XILINX_VERSION/bin"
			make
		fi

		if [[ "$response_synthesize" == *"2"* ]]; then
			cd "$ADI_HDL_PROJECT/projects/ettus/e310"
			export PATH="$PATH:$XILINX_INSTALL_DIR/Vivado/$XILINX_VERSION/bin"
			make
		fi
	fi
